...
static void hash_list_destructor(const void *data)
{
	hash_map_pair *pair = (hash_map_pair *)data;

	safe_free(pair->value);
	safe_free(pair);
	return;
}
...
void hash_map_free(hash_map *map) {
	for (size_t i = 0; i < map->capacity; i++) {
		if (map->table[i]) {
			map->table[i]->free_data = hash_list_destructor;
...
void hash_map_work(hash_map *map, hash_map_loop loop)
{
    unsigned long l;
	linked_list *list;
    linked_list_node *head, *next;
    hash_map_pair *pair;

    if (NULL == loop)
        return;

    for (l=0; l<map->capacity; l++) {
	    if (NULL == (list = map->table[l]))
            continue;
        head = linked_list_head(list); 
        while (head) {
            pair = (hash_map_pair *)head->data;
            next = head->next;
            (*loop)(map, pair->key, pair->value);
            head = next;    
        }
    }
